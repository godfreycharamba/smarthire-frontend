...
const closeModal = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      ...
      case 'reviewed': return <ClockIcon className="h-4 w-4" />;
      case 'shortlisted': return <Calendar className="h-4 w-4" />;
  ...
  const getProgressWidth = (status: string) => {
    switch(status) {
      case 'submitted': return 'w-1/4';
      case 'reviewed': return 'w-1/2';
      case 'shortlisted': return 'w-2/3';
      case 'interviewed': return 'w-3/4';
      case 'offered': return 'w-full';
  ...
  const getProgressLabel = (status: string) => {
    switch(status) {
      ...
      case 'reviewed': return 'Under Review';
      case 'shortlisted': return 'Shortlisted';
      case 'interviewed': return 'Interview Stage';
  ...
  const statusOptions = ['All', 'submitted', 'reviewed', 'shortlisted', 'interviewed', 'offered', 'rejected'];

  // Filter applications
  const filteredApplications = applications.filter(app => {
    const matchesSearch = app.job.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          app.job.employer_profile.company_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-yellow-700">
            {applications.filter(app => app.status === 'reviewed' || app.status === 'shortlisted' || app.status === 'interviewed').length}
          </p>
        </div>
            ...
            >
              {statusOptions.map(status => (
                <option key={status} value={status}>
                  {status === 'All' ? 'All' : getDisplayStatus(status)} ({getStatusCount(status)})
                </option>
              ))}
            </select>
        ...
        <>
          <div className="space-y-4">
            {currentApplications.map((application) => (
              <div key={application.application_id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                <div className="p-6">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        ...
                        <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 border border-gray-200">
                          {application.job.employer_profile.company_logo_url ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-lg">
                              {application.job.employer_profile.company_name.charAt(0)}
                            </div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <p className="text-gray-600 text-sm">{application.job.employer_profile.company_name}</p>
                            </div>
                          </div>
                    ...
                    <div className="flex flex-col items-end space-y-3">
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => handleViewDetails(application)} className="px-4 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                          View Details
                        </button>
          ...
          {filteredApplications.length > applicationsPerPage && (
            <div className="flex items-center justify-between pt-6 border-t border-gray-200">
              <p className="text-sm text-gray-500">
                Showing {indexOfFirstApp + 1}-{Math.min(indexOfLastApp, filteredApplications.length)} of {filteredApplications.length} applications
              </p>
              <div className="flex items-center space-x-2">
                <button 
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-200 p-6 flex items-center justify-between rounded-t-2xl z-10">
        <div className="flex items-center space-x-4">
          {selectedApplication.job.employer_profile.company_logo_url ? (
            <img
              src={selectedApplication.job.employer_profile.company_logo_url}
            ...
            <div className="w-16 h-16 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-2xl">
              {selectedApplication.job.employer_profile.company_name.charAt(0)}
            </div>
          )}
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{selectedApplication.job.title}</h2>
            <p className="text-gray-600">{selectedApplication.job.employer_profile.company_name}</p>
          </div>
        </div>
        <button
          onClick={closeModal}
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-blue-600">{selectedApplication.total_match_score}%</span>
          </div>
          <div className="mt-2 grid grid-cols-3 gap-4">
            <div>
              <p className="text-xs text-gray-500">Skills</p>
              <p className="font-medium text-gray-900">{selectedApplication.skills_score}%</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Experience</p>
              <p className="font-medium text-gray-900">{selectedApplication.experience_score}%</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Education</p>
              <p className="font-medium text-gray-900">{selectedApplication.education_score}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="font-medium text-gray-900 flex items-center">
              ...
              <span className="truncate">{selectedApplication.job.location}</span>
            </p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="font-medium text-gray-900 flex items-center">
              ...
              <span className="truncate">{selectedApplication.job.salary}</span>
            </p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
        ...
        <div>
          ...
          <p className="text-gray-700 leading-relaxed break-words">{selectedApplication.job.description}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-1">
              {selectedApplication.job.required_skills.split(',').map((skill, index) => (
                <span key={index} className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-lg font-medium break-words">
                  {skill.trim()}
                </span>
              ))}
            </div>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            ...
            <p className="text-gray-700 text-sm break-words">{selectedApplication.job.required_experience}</p>
          </div>
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <p className="text-gray-700 text-sm break-words">{selectedApplication.job.required_education}</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex items-center space-x-2 text-gray-600 mb-2">
                ...
                <span className="font-medium">{selectedApplication.applicant.user.first_name} {selectedApplication.applicant.user.last_name}</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600 mb-2">
                <Mail className="h-4 w-4 flex-shrink-0" />
                <span className="text-sm break-words">{selectedApplication.applicant.user.email}</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
                <Phone className="h-4 w-4 flex-shrink-0" />
                <span className="text-sm break-words">{selectedApplication.applicant.user.phone_number}</span>
              </div>
            </div>
            <div>
              <div className="flex items-center space-x-2 text-gray-600 mb-2">
              ...
              <div className="flex items-center space-x-2 text-gray-600">
                ...
                <span className="text-sm">Applied: {formatDate(selectedApplication.applied_date)}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex items-center space-x-2 text-gray-600 mb-2">
                ...
                <span className="font-medium break-words">{selectedApplication.job.employer_profile.company_name}</span>
              </div>
              <p className="text-gray-700 text-sm break-words">{selectedApplication.job.employer_profile.company_description}</p>
            </div>
            <div className="space-y-2">
              <div className="flex items-center space-x-2 text-gray-600">
                <Globe className="h-4 w-4 flex-shrink-0" />
                <a href={selectedApplication.job.employer_profile.company_website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm break-words">
                  {selectedApplication.job.employer_profile.company_website}
                </a>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
                <Mail className="h-4 w-4 flex-shrink-0" />
                <span className="text-sm break-words">{selectedApplication.job.employer_profile.user.email}</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
                <Phone className="h-4 w-4 flex-shrink-0" />
                <span className="text-sm break-words">{selectedApplication.job.employer_profile.user.phone_number}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 pt-4">
          <div className="flex items-center text-gray-600">
            ...
            <span className="ml-2 break-words">{formatDate(selectedApplication.job.deadline)}</span>
          </div>
        </div>
      </div>
    </div>
